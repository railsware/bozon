...
describe '#bozon', ->
  ...
  describe '#dest', ->
    ...
    it 'should set gulp dest', =>
      ...
      expect(gulpDestSpy.getCall(0).args[0]).to.eq("#{process.cwd()}/builds/development/javascripts")
  ...
  describe '#runMocha', =>
    ...
    it 'should run mocha with arguments', =>
      ...
      expect(childProcessSpy.getCall(1).args[0]).to.eq("#{process.cwd()}/node_modules/.bin/mocha")
      expect(childProcessSpy.getCall(1).args[1]).to.eql(['--recursive', 'spec/unit/some_spec.js'])
  ...
  describe '#runElectron', =>
    ...
    it 'should run electron for build directory', =>
      ...
      expect(childProcessSpy.getCall(2).args[0]).to.eq("#{process.cwd()}/node_modules/.bin/electron")
      expect(childProcessSpy.getCall(2).args[1]).to.eql(["#{process.cwd()}/builds/development"])
