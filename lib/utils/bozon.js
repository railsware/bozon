...
const bozon = {
  ...
  config: function(env, platform) {
    ...
    this.config.file(this.source('config', 'settings.json'))
    this.config.file(this.source('config', 'environments', env + '.json'))
    this.config.file(this.source('config', 'platforms', platform + '.json'))
